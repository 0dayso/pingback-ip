...
namespace gs.DataBase
{
	...
	public class Top3:Conn
	{
		...
		public Top3(string p_strConnStr)
		{
			m_Ary = null;
		...
		public bool Update(string p_strTableName,string p_strWhere)
		{
			...
			if(bRet)
			{
				...
				{
					bRet = false;
					gs.util.func.Write("top 3 2 is err" + ex.Message);
					throw ex;
				}
			}
			else
		...
		/// <returns></returns>
		public bool Delete(string p_strTableName,string p_strWhere)
		{
			...
			{
				bRet=false;
				gs.util.func.Write("top3 del is err" + ex.Message);
