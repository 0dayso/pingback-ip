...
namespace EagleString
{
    ...
    {
        public static bool LargeThan420(DateTime dt)
        {
            if (DateTime.Compare(dt, DateTime.Parse("2009-4-20"))>=0)
            {
                return true;
            }
            return false;
        ...
        public static string CreateXmlOfEagleIbe(string[] flightno, string[] actioncode, DateTime[] date, string[] city, string[] city2, char[] bunk,
    string[] names, DateTime dtlimit, string[] cardnos, string[] remarks)
        {
            string xml = "<?xml version=\"1.0\" encoding=\"utf-8\" ?><eg>{0}</eg>";
            string temp = "<rec><IdCard>证件号</IdCard><AirCorp>YY</AirCorp><IdType>NI</IdType><CustName>姓名</CustName><CustBirthday>生日</CustBirthday></rec>";
            string nm = "<record>";
            for (int i = 0; i < names.Length; i++)
            {
                nm += temp.Replace("证件号", cardnos[i]).Replace("姓名", names[i]).Replace("生日","");
            }
            nm += "</record>";
            string seg = "<segs>";
            temp = "<seg><Air>航班号</Air><ActionCode>LL</ActionCode><FlyTime>航班日期YYYYMMDD</FlyTime><Org>出发城市</Org><Dst>到达城市</Dst><Cabin>舱位</Cabin></seg>";
            for (int i = 0; i < flightno.Length; i++)
            {
                seg += temp
                    .Replace("航班号", flightno[i])
                    .Replace("航班日期YYYYMMDD", date[i].ToString("yyyyMMdd"))
                    .Replace("出发城市", city[i])
                    .Replace("到达城市", city2[i])
                    .Replace("舱位", bunk[i].ToString());
            }
            seg += "</segs>";

            temp = "<EtkPrc></EtkPrc><Fule></Fule><BaseFee></BaseFee><PassType>Adult</PassType><TkDate>出票时限</TkDate><PerCount>人数</PerCount>";
            string price = temp.Replace("出票时限", dtlimit.ToString("yyyyMMdd HH:mm:ss")).Replace("人数", names.Length.ToString());
            xml = string.Format(xml, nm + seg + price);
            return xml;
        }
        ...
        /// <param name="s"></param>
        /// <returns></returns>
        public static string cutEtermHead_1B4D(string s)
        {
            try
            {
        ...
        /// <summary>
        /// 若str为NULL，则返回""，否则返回原值
        /// </summary>
        /// <param name="str">为string 类型的object</param>
        /// <returns></returns>
        public static string Null2S(object str)
        {
            if (str == null) return "";
            return str.ToString();
        }
        public static System.Globalization.DateTimeFormatInfo dtFormat = new System.Globalization.CultureInfo("en-us", false).DateTimeFormat;
        ...
        static public List<string> ArrayToList(string[] arr)
        {
        ...
        static public void sortStringListByPinYin(List<string> lsName)
        {
            try
            {
                ...
                {
                    lsName[i] = GetSpell(lsName[i]) + "~" + lsName[i];
                }
                lsName.Sort();
                for (int i = 0; i < lsName.Count; i++)
                {
            ...
            {
                lsName.Sort();
                EagleFileIO.LogWrite("姓名排序时发生错误:" + string.Join(",", lsName.ToArray()) + ex.Message);
            }
        }
        static public void sortNameAndCardList(List<string> lsName, List<string> lsCard)
        {
            try
            {
                for (int i = 0; i < lsName.Count; i++)
                {
                    lsName[i] = GetSpell(lsName[i]) + "~" + lsName[i] + "~" + lsCard[i];
                }
                lsName.Sort();
                for (int i = 0; i < lsName.Count; i++)
                {
                    lsCard[i] = lsName[i].Split('~')[2];
                    lsName[i] = lsName[i].Split('~')[1];
                }
            }
            catch (Exception ex)
            {
                System.Windows.Forms.MessageBox.Show
                    ("姓名排序时发生错误，请将日志文件Email至clawclaw@163.com，并说明问题");
                EagleFileIO.LogWrite("姓名排序时发生错误:" + string.Join(",", lsName.ToArray()) + ex.Message);
            }
        }
        /// <summary>
        /// 计算对应折扣的票面价
        /// </summary>
        /// <param name="yPrice">Y舱价格</param>
        /// <param name="rebate">折扣 0-150</param>
        static public int TicketPrice(int yPrice, int rebate)
        {
            int scale = 1000;
            int temp = yPrice * rebate;
            int ys = temp % scale;
            if (ys < (scale/2)) temp = temp - ys;
            else temp = temp + scale - ys;
            return temp * 10 / scale; 
        }
        /// <summary>
        /// 在t中取s1与s2之间的字符串，若找不到则返回为""
        /// </summary>
        /// <param name="s1">前串(从头开始搜索)</param>
        /// <param name="s2">后串(从s1开始往后搜索)</param>
        static public string Between2String(string t, string s1, string s2)
        {
            int p = t.IndexOf(s1);
            int p2 = t.IndexOf(s2, p + s1.Length);
            if (p < 0 || p2 < 0 || p2<p) return "";
            return trim(t.Substring(p + s1.Length, p2 - p - s1.Length));
        }
        /// <summary>
        /// 在t中取s1与s2之间的字符串
        /// </summary>
        /// <param name="s1">后串(从头开始搜索)</param>
        /// <param name="s2">前串(从s1开始往前搜索)</param>
        static public string Between2StringReverse(string t, string s1, string s2)
        {
            int p = t.IndexOf(s1);
            int p2 = t.LastIndexOf(s2, p-1);
            if (p < 0 || p2 < 0 || p2 > p) return "";
            return trim(t.Substring(p2 + s2.Length, p - p2 - s2.Length));
        }
        /// <summary>
        /// 取t中最后一个s之后的剩余串，若不包含s，则返回""
        /// </summary>
        /// <param name="s">t中的最后一个s</param>
        static public string AfterString(string t, string s)
        {
            int pos =0;
            //if ((pos = t.IndexOf(s)) < 0) return "";
            if ((pos = t.LastIndexOf(s)) < 0) return "";//modified by king
            return t.Substring(pos + s.Length);
        }
        static public void Switch(ref string A, ref string B)
        {
            string temp = A;
            A = B;
            B = temp;
    ...
    public class egXmlString
    {
        /// <summary>
        /// get the Name/Value of the specified node in the xml
        /// </summary>
        /// <param name="xml"></param>
        /// <param name="nodepath">like "//eg/result"</param>
        /// <returns></returns>
        static public Hashtable ToHashTable(string xml, string nodepath)
        {
            ...
            {
                ht.Add(xn.Name, xn.Value);
            }
            else
            {
                for (int i = 0; i < xn.ChildNodes.Count; ++i)
                {
                    ht.Add(xn.ChildNodes[i].Name, xn.ChildNodes[i].InnerXml);
                }
            }
            return ht;
        }
