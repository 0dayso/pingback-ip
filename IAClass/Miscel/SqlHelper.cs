...
public sealed class SqlHelper
{
    ...
    //public static readonly string connectionString = System.Configuration.ConfigurationManager.ConnectionStrings["testConnectionString"].ConnectionString;
    ...
    /// <returns></returns>
    private static SqlParameter CreateParam(string param, object obj)
    {
    ...
    /// <param name="commandText">存储过程名或都T-SQL命令文本</param>
    /// <param name="commandParameters">和命令相关联的SqlParameter参数数组,如果没有参数为'null'</param>
    /// <param name="paraNames">参数名称</param>
    /// <param name="paraValues">值</param>
    /// <param name="mustCloseConnection"><c>true</c> 如果连接是打开的,则为true,其它情况下为false.</param>
    private static void PrepareCommand(SqlCommand command, SqlConnection connection, SqlTransaction transaction, CommandType commandType, string commandText, string[] paraNames, object[] paraValues, out bool mustCloseConnection)
    {
        if (command == null) throw new ArgumentNullException("command");
        if (commandText == null || commandText.Length == 0) throw new ArgumentNullException("commandText");
        ...
        if (paraNames != null && paraValues != null)
        {
            ...
            {
                SqlParameter p = CreateParam(paraNames[i], paraValues[i]);
                command.Parameters.Add(p);
            }
        }
        return;
    ...
    public static int ExecuteNonQuery(string connectionString, CommandType commandType, string commandText, string[] paraNames, object[] paraValues)
    {
        // ExecuteNonQuery #2
        if (connectionString == null || connectionString.Length == 0) throw new ArgumentNullException("connectionString");
        ...
        {
            connection.Open();
            return ExecuteNonQuery(connection, commandType, commandText, paraNames, paraValues);
        }
    }
    ...
    public static int ExecuteNonQuery(SqlConnection connection, CommandType commandType, string commandText, string[] paraNames, object[] paraValues)
    {
        // ExecuteDataset #5
        ...
        PrepareCommand(cmd, connection, (SqlTransaction)null, commandType, commandText, paraNames, paraValues, out mustCloseConnection);
    ...
    public static DataSet ExecuteDataset(string connectionString, CommandType commandType, string commandText, string[] paraNames, object[] paraValues)
    {
        // ExecuteDataset #2
        if (connectionString == null || connectionString.Length == 0) throw new ArgumentNullException("connectionString");
        ...
        {
            connection.Open();
            return ExecuteDataset(connection, commandType, commandText, paraNames, paraValues);
        }
    }
    ...
    public static DataSet ExecuteDataset(SqlConnection connection, CommandType commandType, string commandText, string[] paraNames, object[] paraValues)
    {
        // ExecuteDataset #5
        if (connection == null) throw new ArgumentNullException("connection");
    ...
    public static object ExecuteScalar(string connectionString, CommandType commandType, string commandText, string[] paraNames, object[] paraValues)
    {
        // ExecuteScalar #2
        if (connectionString == null || connectionString.Length == 0) throw new ArgumentNullException("connectionString");
        // 创建并打开数据库连接对象,操作完成释放对象.
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            connection.Open();
            // 调用指定数据库连接字符串重载方法.
            return ExecuteScalar(connection, commandType, commandText, paraNames, paraValues);
        }
    }
    ...
    public static object ExecuteScalar(SqlConnection connection, CommandType commandType, string commandText, string[] paraNames, object[] paraValues)
    {
        // ExecuteScalar #5
        if (connection == null) throw new ArgumentNullException("connection");
        // 创建SqlCommand命令,并进行预处理
